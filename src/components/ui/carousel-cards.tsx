...
export const Carousel = ({ items }: CarouselProps) => {
	...
	const fullCardSize = cardWidth + gap;
	...
	useEffect(() => {
		...
		const handleScroll = () => {
			...
			const index = Math.floor(container.scrollLeft / fullCardSize);
	...
	return (
		...
		>
			<div className="relative w-full">
				<div
					className="relative w-full overflow-x-scroll scroll-smooth py-10 [scrollbar-width:none] md:py-20"
					ref={carouselRef}
				>
					<div
						className={cn(
	...
	return (
		<>
			...
			<motion.button
				layoutId={layout ? `card-${card.title}` : undefined}
				onClick={handleOpen}
				className="relative z-10 flex h-80 w-56 flex-col items-start justify-start overflow-hidden rounded-3xl bg-gray-100 hover:cursor-pointer md:h-[40rem] md:w-96 dark:bg-neutral-900"
			>
